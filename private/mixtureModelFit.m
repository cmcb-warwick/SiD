...
optoptions = optimset('Jacobian','on','Display','off','Tolfun',1e-5,'TolX',1e-6);
alphaA = options.mmfSensitivity; % amplitude t-test cutoff.
alphaD = 0.01;
is3D = ndims(image) == 3;
if is3D
  degFreePerSpot = 4;
...
% Distance criterion for defining separate clusters - default = 5µm
clusterSep = 5*psfSigma;

% Cluster spots.
clusters = clusterSpots(cands);
nClusters = max(clusters);
if verbose
  sidLog('Mmf fitting on %d clusters',nClusters);
end
spots = [];
amps = [];
bgAmps = [];
...
for i=1:nClusters
  ...
  idx = clusters==i;
  clusterCandsT = cands(idx,:); % Candidates in this cluster.
  if is3D
  ...
  numCands = size(clusterCandsT,1);
  if verbose
    sidLog('Fitting cluster %d, %d cands',i,numCands);
  end
  ...
  % Fit mixture-model.
  [solution,resnorm,~,~] = fitNGaussiansFitFun(optoptions,x0,lb,ub,...
  ...
  if options.mmfMaxTime > 0 && etime(clock,startTime) > options.mmfMaxTime
    spots = []; amps = []; bgAmps = [];
    return % Abort
  end
...
clusters = clusterSpots(spots);
nClusters = max(clusters);
if verbose
    sidLog('Reclustering produced %d clusters',nClusters);
end
...
for i=1:nClusters
  ...
  [bgAmp,clusterCands,clusterAmp] = extractSolution(solution,numCands);
  if verbose
      sidLog('Refit to new cluster, %d cands',numCands);
  end
  ...
  while numCands > 1
    ...
    if ~testDist
      if verbose
        sidLog('All candidates passed distance test. Range of p=[%g,%g]', ...
    ...
    if numCands == 0
        if verbose
          sidLog('All candidates removed due to failing distance test');
        end
        break;
    end
    ...
    if verbose
      sidLog('Removed candidate failing distance test, p=%f, %d cands',pValueMax,numCands);
    end
  ...
  while numCands > 0
    ...
    if ~testAmp
      if verbose
        sidLog('All candidates passed amplitude test. Range of p=[%g,%g]',min(pValue),pValueMax);
      end
      break;
    end
    ...
    if numCands == 0
      if verbose
        sidLog('All candidates removed due to failing ampltitude test');
      end
      break;
    end
    ...
    if verbose
      sidLog('Removed candidate failing amplitude test, p=%f, %d cands',pValueMax,numCands);
    end
  end
  ...
  if verbose
    sidLog('Cluster fitting complete, %d spots',numCands);
  end
...
if verbose
  sidLog('MMF complete, %d spots',size(spots,1));
end
