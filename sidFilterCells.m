function sidFilterCells(jobset)
% SIDFILTERCELLS(EXPTS) Allows the user to remove cells whose intensities
...
detC = jobset.options.detectChan;
meaC = find(jobset.options.measureChans);
nChans = length(meaC);
chanOrder = [2 1 3];
...
while ~handles.stop
    ...
    [~,reader] = sidOpenMovie(fullfile(job{iImg}.movieDirectory,job{iImg}.ROI.movie),'ROI');
    imgSize = sidComputeStackSize(job{iImg}.ROI.crop,job{iImg}.metadata.frameSize);
    rgbImg = zeros([imgSize(1:2) 3]);
    for iChan = 1:nChans
      img = sidReadImageStack(reader,job{iImg}.metadata,meaC(iChan),job{iImg}.ROI.crop,0);
...
sidLog('Manual cell filtering complete');
close(gcf);
