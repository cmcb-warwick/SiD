function varargout=sidOpenMovie(movieFileName,mode,metadata,suppWarn)
% SIDOPENMOVIE Open movie and extract metadata from BioFormats metadata
%
%    [METADATA,READER] = SIDOPENMOVIE(MOVIEFILENAME) Open movie with filename
...
if mode == 1
  sidLog('Opening movie: %s', movieFileName);
end
...
try
    ...
    md.wavelength = [488 561 705 360]/1000; % Default assumes EGFP, mCherry,
                                        % DAPI. FIXME Ask user.
    warnWv = 0;
    for i=1:numWvs
